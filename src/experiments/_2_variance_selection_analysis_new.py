...
# -------------------------
DEFAULT_N_BOOTSTRAP_SAMPLES = 5 
DEFAULT_N_CANDIDATE_SUBSETS = 5
DEFAULT_TOTAL_ANNOTATIONS = 300
DEFAULT_DATASET = "summeval"
...
DEFAULT_PLOTS_DIR = f"results/04_29_{DEFAULT_DATASET}"
...
def compute_variance_alignment(df, target_models, ensemble_models, mode="aggregate"):
    ...
    if mode == "aggregate":
        ...
        for m in target_models:
            ...
            # hm_fallback = compute_icc_pingouin(df[df["model_name"].isin([m, "original"])])
            try:
                hm_msb = hm_icc_obj.msb
                hm_mse = hm_icc_obj.mse
                hm_msb_list.append(hm_msb)
...
def _run_trials_for_base(base_strategy, strategy_variants, text_ids, k, n_trials,
    ...
    if base_strategy == "random":
        print(f"      _run_trials_for_base: base_strategy={base_strategy}, k={k}, n_trials={n_trials}, actual_trials={actual_trials}")
    ...
    for trial_idx in range(actual_trials):
        if base_strategy == "random" and trial_idx == 0:
            print(f"        Starting trial {trial_idx} for random")
        seed = 42 + trial_idx
        ...
        try:
            if needs_plain:
                ...
                plain_msre = compute_mean_sq_err(hm_sample)

            if needs_ppi:
                ppi_icc, ppi_alpha, ppi_rho, ppi_tau, ppi_msre = compute_reliability_ppi_corrected(
                    hm_sample, im_full_df, true_im_icc, true_im_alpha, im_standalone_mse
                    hm_models=[model, "original"], im_models=im_models
                )
        except Exception as e:
            if base_strategy == "random":
                print(f"        EXCEPTION in trial_idx={trial_idx}, k={k}: {type(e).__name__}: {e}")
            continue
        ...
        # if base_strategy == "random":
        #     print("BASE STRATEGY:", base_strategy)
        ...
        for strategy in strategy_variants:
            ...
            if base_strategy == "random" and trial_idx == 0 and k == 5:
                print(f"        DEBUG trial_idx={trial_idx}, k={k}, strategy={strategy}")
                print(f"          matched_metric={matched_metric}")
                print(f"          est_rho={est_rho}, true_rho={true_rho}")
                print(f"          est_rho is not None: {est_rho is not None}")
                print(f"          np.isfinite(est_rho): {np.isfinite(est_rho) if est_rho is not None else 'N/A'}")
                print(f"          true_rho is not None: {true_rho is not None}")
                print(f"          np.isfinite(true_rho): {np.isfinite(true_rho) if true_rho is not None else 'N/A'}")
            ...
            if matched_metric in (None, "rho"):
                ...
                elif base_strategy == "random" and trial_idx == 0 and k == 5:
                    print(f"          RHO CHECK FAILED for {strategy}")
...
def evaluate_reliability_estimators(df, target_models, ensemble_models, per_model_variance,
    ...
    print(f"\n=== STRATEGIES GROUPED BY BASE ===")
    for base, variants in strategies_by_base.items():
    ...
    for model in target_models:
        ...
        for base_strategy, strategy_variants in strategies_by_base.items():
            print(f"\n=== Processing base_strategy: {base_strategy}, variants: {strategy_variants}, model: {model} ===")
            past_im_msb_obs = []
            past_im_mse_obs = []
            past_hm_msb_obs = []
            past_hm_mse_obs = []
            ...
            for k in budgets:
                ...
                print(f"    trial_results.keys() = {list(trial_results.keys())}")
                for strategy, errors in trial_results.items():
                    print(f"      strategy={strategy}: icc_errors={len(errors['icc_errors'])}, alpha_errors={len(errors['alpha_errors'])}, rho_errors={len(errors['rho_errors'])}, tau_errors={len(errors['tau_errors'])}")
    ...
    print(f"Total icc_results: {len(icc_results)}")
    print(f"Total alpha_results: {len(alpha_results)}")
    print(f"Total rho_results: {len(rho_results)}")
    print(f"Total tau_results: {len(tau_results)}")
    if rho_results:
        rho_df = pd.DataFrame(rho_results)
        print(f"Rho methods: {rho_df['method'].unique()}")
        print(f"Rho method counts: {rho_df['method'].value_counts().to_dict()}")
...
def _run_axis_worker(args):
    ...
    axis, axis_df = args
    print(f"\n{'='*60}")
    print(f"WORKER: Starting axis '{axis}'")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"WORKER: Completed axis '{axis}'")
    print(f"  rho_results: {len(axis_rho)} rows")
    if len(axis_rho) > 0:
        print(f"  rho methods: {axis_rho['method'].unique()}")
    print(f"{'='*60}")
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"MAIN: Received {len(axis_results)} axis results")
    for i, (axis, axis_icc, axis_alpha, axis_mse, axis_rho, axis_tau, _, _) in enumerate(axis_results):
        print(f"  [{i}] axis='{axis}': rho={len(axis_rho)} rows")
        if len(axis_rho) > 0:
            print(f"      rho methods: {axis_rho['method'].unique()}")
    print(f"{'='*60}")
